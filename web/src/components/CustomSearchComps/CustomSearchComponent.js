...
import {Row,Col, Button, Card, Input,Checkbox, Tag, Typography, Slider,Switch, message, Table, Tabs, InputNumber} from 'antd'
...
export default class CustomSearchPage extends React.Component{
     constructor(props){
         ...
         this.state = {
            ...
            size:3,
            data:{
                drugs:{label:'Drugs',url:`${options.uri}drugs`, result:[], columns:columns('drug')}, 
                diseases:{label:'Diseases', url:`${options.uri}diseases`, result:[],  columns:columns('disease')}, 
                texts:{label:'Articles', url:`${options.uri}texts`, result:[]}
            }
            }
         }
     alterUrls = (keys) => {
     ...
     changeAtcLevel = val => {this.setState({atcLevel:val})}
     changeMeshLevel = val => {this.setState({meshLevel:val})}
     getLevelParam = (key) => {
         let result = '' 
     ...
     componentDidMount(){
        if(this.props.data.keywords.length != 0){
            this.createParams(this.props.data.keywords)
        }
     ...
     render(){
        return(
            <Card className="shadowEffect">
                ...
                <Row>
                <Col>
        <Button onClick={() => this.ChangeShowQueryState()}>{this.state.showQuery?'Hide':'Show'} Query {this.state.showQuery?<EyeInvisibleOutlined/>:<EyeOutlined/>}</Button>
                </Col>
                </Row>
                ...
                return(
                    <div>
                        <Row align="middle" gutter={[8,8]}>
                            ...
                            <Col>
                                <Button onClick={() => this.sendRequest(key)}>Run request</Button>
                            </Col>
                        </Row>
